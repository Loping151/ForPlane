...
<<<<<<< HEAD
        ...
        if dset_type == "llff":
            ...
            # PYTHONPATH='.' python lerplanes/main.py --config-path lerplanes/configs/final/endo_hybrid.py
            # if split == "render":
            if False:  # always use all data
                ...
                poses, _, intrinsics = load_llff_poses_helper(
...
class Video360Dataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if dset_type == "llff":
            ...
            if split == 'render':
                num_tsteps = 120
                dnerf_durations = {'hellwarrior': 100, 'mutant': 150, 'hook': 100,
                                   'bouncingballs': 150, 'lego': 50, 'trex': 200, 'standup': 150, 'jumpingjacks': 200}
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            num_frames_per_camera = len(
                self.imgs) // (len(self.per_cam_near_fars) * h * w)
            camera_id = torch.div(
                image_id, num_frames_per_camera, rounding_mode='floor')  # (num_rays)
            out['near_fars'] = self.per_cam_near_fars[camera_id, :]
        else:
            out['near_fars'] = self.per_cam_near_fars  # Only one test camera
